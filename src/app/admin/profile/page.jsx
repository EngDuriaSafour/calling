...
const Profile = () => {
  ...
  const closeAdminAccount = async () => {
    try {
      if (confirm("Yönetici hesabından çıkış yapmak istediğinize emin misiniz?")) {
        const res = await axios.put(`${process.env.NEXT_PUBLIC_API_URL}/admin`);
        if (res.status === 200) {
          router.push("/admin");
          toast.success("Yönetici oturumu kapatıldı!");
        }
      }
    } catch (err) {
      console.log(err);
  ...
  return (
    ...
    <div className="flex px-10 min-h-[calc(100vh_-_433px)] lg:flex-row flex-col lg:mb-0 mb-10 relative">
      <div className="lg:w-80 w-100 flex-shrink-0">
        ...
        <ul className="text-center font-semibold">
          ...
          >
            ...
            <button className="ml-1">Alt Bilgi (Footer)</button>
          </li>
          <li
            className={`border w-full p-3 cursor-pointer hover:bg-primary hover:text-white transition-all ${
